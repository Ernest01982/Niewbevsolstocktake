...
This matrix separates tests automated through Phase 6 from mandatory later-phase coverage. A later-phase row is not marked passed merely because its table or workflow does not yet exist.

## Phase 0 through Phase 6 automated tests
...
| P6-SCHEMA-01 | Schema        | Threshold, batch, task, and recount-result objects     | Exist with forced RLS and constraints              | pgTAP         |
| P6-INDEX-01  | Performance   | Every Phase 6 composite foreign key                    | Covered in foreign-key column order                | pgTAP/advisor |
| P6-THRESH-01 | Precedence    | Product, warehouse, and company thresholds overlap     | Product → warehouse → company                      | pgTAP         |
| P6-THRESH-02 | Authorisation | Manager or Stock Taker changes threshold policy        | Structured forbidden result                        | pgTAP         |
| P6-PROG-01   | Progress      | Duplicate/count-type records cover one product         | Distinct progress capped at 100%                   | pgTAP         |
| P6-VAR-01    | Variance      | Initial immutable counts are compared with snapshot    | Derived signed/absolute units                      | pgTAP         |
| P6-VAR-02    | Recount       | Completed recount exists                               | Latest recount is effective physical               | pgTAP         |
| P6-FLAG-01   | Resolution    | Direct flag update/delete                              | Rejected by narrow trigger                         | pgTAP         |
| P6-FLAG-02   | Audit         | Manager resolves flag with required note               | One controlled transition and audit                | pgTAP         |
| P6-REC-01    | Generation    | Above-threshold filtered product                       | One open task generated                            | pgTAP         |
| P6-REC-02    | Assignment    | Manager assigns/returns task to pool                   | Valid active Stock Taker only                      | pgTAP         |
| P6-REC-03    | Claim         | Two Stock Takers claim one pool task                   | First wins; second receives unavailable            | pgTAP         |
| P6-REC-04    | Blind data    | Stock Taker requests recount work                      | No SOH, variance, threshold, prior count           | pgTAP         |
| P6-REC-05    | Submission    | Claimed full-product recount                           | Server total, immutable, request-bound idempotency | pgTAP         |
| P6-RLS-01    | Tenancy       | Cross-company/unallocated/anonymous Phase 6 reads      | Zero rows or permission denied                     | pgTAP         |
| P6-CLOSE-01  | Completion    | Open recount task or count flag remains                | Finalisation blocked                               | pgTAP         |
