# Phase 1 through Phase 6 RLS design
...
| `warehouse_settings`                | Authorised company warehouses                       | Allocated warehouses                             | Allocated warehouse                       | Denied                    |
| `product_warehouse_settings`        | Authorised company warehouses                       | Allocated warehouses                             | Allocated warehouse                       | Denied                    |
| `recount_batches` / `recount_tasks` | Authorised company warehouses                       | Allocated warehouses                             | Allocated warehouse                       | Denied                    |
| `recount_counts`                    | Authorised company warehouses                       | Allocated warehouses                             | Allocated warehouse                       | Own submitted results     |
...
Phases 1 through 6 grant no direct INSERT, UPDATE, or DELETE access to the frontend for tenant, membership, product, import, snapshot, lifecycle, session, count, flag, recognition, threshold, recount-control, recount-result, or audit tables. Development fixtures run as the database owner. Imports, lifecycle actions, count sync, recognition, threshold, flag-resolution, and recount operations call narrow transactional functions with actor, scope, validation, concurrency, idempotency, and audit behavior tested together.
...
Phase 6 management RPCs derive progress and variance inside the database and require allocated Admin/Manager access. Only Admin or explicitly authorised Super Admin paths may change threshold policy. Recount generation and assignment are management-only; Stock Takers receive a separate blind work RPC, atomically claim one eligible task, and submit one idempotent immutable result through a server-calculated packaging path. Direct task mutation and result insertion remain denied.

## Restricted stock data

SOH snapshot rows, count flags, threshold settings, recount batches, and recount control tasks receive no Stock Taker RLS policy, so direct queries return zero rows. The stock-taker context RPC returns only session, company, warehouse, and stock-take identity/status. The blind recount RPC returns task identity plus product/search/packaging data, but never snapshot, prior count, source physical, variance, or threshold fields. Tests assert both zero-row direct access and forbidden-field absence. A Stock Taker may read only their own submitted initial/recount results. Filtering restricted fields in the UI is prohibited as a control.

## Policy review checklist

- RLS enabled and forced on every exposed Phase 1–6 table.
